...
class Stemmer {
   ...
   */
    public static String stemString(String term) { 
      ...
      char[] stemmingChar = new char[term.length()];
      int j = 0;
      ...
      for(int k=0;k<j;k++) stemmer.add(stemmingChar[k]);
      stemmer.stem();
      return stemmer.toString();
    }
    
    public String stemStringInstance(String term) {
      char[] stemmingChar = new char[term.length()];
      int j = 0;
      for( int i=0; i<term.length(); i++ ) {
        char ch = term.charAt(i);
      ...
      for(int k=0;k<j;k++) add(stemmingChar[k]);
      stem();
  ...
  {
    /* stemming method once used
    char[] stemmingChar = new char[501]; // temporarily store a word in the
    // string
    try {
    ...
    for (int docId = 0; docId < files.size(); docId++) {
      ...
      try {
        doc = Jsoup.parse(files.get(docId), "UTF-8", "");
      } catch (IOException e) {
        System.err.println(e.getMessage());
      }
      ...
      // Stemming the title of the document and put the stemmer into a list
      for (int i = 0; i < docTitle.length(); i++) {
        char ch = docTitle.charAt(i);
        if (Character.isLetter(ch)) {
          int j = 0; // stands for the end of a word
          while (true) {
            stemmingChar[j] = ch;
            if (j < 500)
              j++;
            // if it's not the end of the string
            if (i < docTitle.length() - 1) {
              ch = docTitle.charAt(++i);
              if (!Character.isLetter(ch)) {
                for (int k = 0; k < j; k++)
                  stemmer.add(stemmingChar[k]);
                stemmer.stem();
                String stemmerResult = stemmer.toString(); // the result of the
                // stemming
                if (Compress.isEnglishWord(stemmerResult)) {
                  if (!StopWordsList.isStopWord(stemmerResult)) {
                    docTitleList.add(stemmerResult);
                  }
                }
                break;
              }
      ...
      // stem the body of the document and put the result into a list
      for (int i = 0; i < docBody.length(); i++) {
        char ch = docBody.charAt(i);
        if (Character.isLetter(ch)) {
          int j = 0; // stands for the end of a word
          while (true) {
            stemmingChar[j] = ch;
