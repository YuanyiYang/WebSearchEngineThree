...
public class IndexerInvertedOccurrence extends Indexer {
  ...
  private final String FINALINDEX = "/data/index/final2.idx";

  private static final long serialVersionUID = -8316859120363593185L;
  ...
  public void constructIndex() throws IOException {
    ...
    String indexFile = _options._indexPrefix + "/indexInvertedOccurrence.idx";
  ...
  private void buildWholeIndexFromPartial() {
    File finalFile = new File(WORKINGDIR + FINALINDEX);
    if (finalFile.exists()) {
      finalFile.delete();
    }
    try {
      ...
      for (int i = 0; i < allTermsSize; i += PROCESSUNIT) {
        ...
        for (int j = 0; j < tempFileName.size(); j++) {
          loadFromTermIdToTermIdtoMemory(i, i + PROCESSUNIT - 1, j,
              tempInvertMap);
          System.out.println("construct index from file " + j);
          System.out.println("map size " + tempInvertMap.size());
        }
        String finalFilePath = WORKINGDIR + FINALINDEX;
        WriteToFile wr = new WriteToFile(finalFilePath);
        wr.appendMapToFile(tempInvertMap);
      }
  ...
  public void loadIndex() throws IOException, ClassNotFoundException {
     ...
     "/indexInvertedOccurrence.idx";
    //String indexFile = "data/index/indexInvertedOccurrence.idx";
  ...
  public void buildInvertMap(Query query) {
    ...
    Map<Integer, Map<Integer, List<Integer>>> tempResult = new HashMap<Integer, Map<Integer, List<Integer>>>();
    try {
      tempResult = buildMap(queryTokens, phrases, fileName);
    } catch (IOException e) {
      e.printStackTrace();
    }
    for (Integer i : tempResult.keySet()) {
      if (!invertedMap.containsKey(i)) {
        invertedMap.put(i, tempResult.get(i));
      }
    }
  ...
  // check
  private Map<Integer, Map<Integer, List<Integer>>> buildMap(
      Vector<String> tokens, Vector<Vector<String>> phrases, String location)
      throws IOException {
    List<Integer> term_id = new ArrayList<Integer>();
    ...
    int index = 0;
    Collections.sort(term_id);

    String line;

    while (index < term_id.size() && (line = br.readLine()) != null) {
      Scanner s = new Scanner(line);
      int t_id = Integer.parseInt(s.next());

      if (term_id.get(index) == t_id) {
        String input = s.next();
        Map<Integer, List<Integer>> temp_map = buildMapList(line);
        r.put(t_id, temp_map);
        index++;
      }
      s.close();
  ...
  Map<Integer, List<Integer>> buildMapList(String input) {
  ...
  private void buildIndex(String directoryPath) {
    ...
    for (int docId = 0; docId < files_size; docId++) {
      ...
      if (docId != 0 && docId % 100 == 0) {
        ...
        String fileName = WORKINGDIR + PARTS + "PartialIndexOccurence"
        ...
        String fileName = WORKINGDIR + PARTS + "PartialIndexOccurenceLast";
        tempFileName.add(fileName);
        WriteToFile wr = new WriteToFile(fileName);
        List<Integer> termIdList = new ArrayList<Integer>(invertedMap.keySet());
        Collections.sort(termIdList);
